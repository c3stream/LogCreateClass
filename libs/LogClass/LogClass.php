...
class LogClass {
    ...
    function __construct($path, $name){
        ...
        if(!touch($this->_file_path)){
            throw new \Exception("DOSE NOT CREATE FILE! :". $this->_file_path);
        }
        if(!$this->_CheckLogFileHealth()){
            throw new \Exception("SETTING ERROR :". $this->_file_path);
        }
        ...
        $this->_my_file_path = $this->_my_class_file_log_path."/".$this->_my_class_file_log_name."_".$this->_log_name_suffix;
        if(!touch($this->_my_file_path)){
            throw new \Exception("DOSE NOT CREATE FILE! :". $this->_my_file_path);
        }
        if(!$this->_CheckMyLogFileHealth()){
            throw new \Exception("SETTING ERROR :". $this->_my_file_path);
        }
    }

    // 参照が何もなくなった時に自動で呼ばれるメソッド
    function  __destruct(){
        $this->_file_path = $this->_my_file_path;
    ...
    private  function _CheckMyLogFileHealth(){
        try{
            if(!file_exists($this->_my_file_path)){
                $this->_my_class_errors[] = "NO FILE EXISTS!SHOULD LOOK FILE PATH : ". $this->_file_path;
                return false;
            } else {
                clearstatcache(TRUE, $this->_my_file_path);
            }
        } catch (\Exception $e){
        ...
        try {
            if(!is_writable($this->_my_file_path)){
    ...
    private  function _CheckLogFileHealth(){
